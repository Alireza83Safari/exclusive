...
import { CommentTableProps } from "./CommentTable.interface";
...
function CommentTable(props: CommentTableProps) {
  const { comments, isLoading, refetchComments } = props;
  ...
  const pageSize = 12;
  const totalPages = Math.ceil(comments?.total / pageSize);
              ...
              <>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    comments?.data.map((row: any, index: any) => (
                      <TableRow key={row.id}>
